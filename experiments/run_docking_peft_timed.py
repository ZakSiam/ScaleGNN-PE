# PhasedGNN-UCB (GNN-PE) on docking (Parquet-derived molecules) with two acceleration axes:
...
#
# With --peft none and no scan flags this reduces to the plain GNN-PE phased runner (the `fullntk`
# arm in the paper). It additionally logs everything the drug-discovery / efficiency metrics
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="PhasedGNN-UCB (GNN-PE) on docking (Parquet-derived molecules) with PEFT + scan acceleration")
    ...
    parser.add_argument("--exp_result_folder", type=str, default="results/docking_3CLPro_dock_norm_peft_T300")
