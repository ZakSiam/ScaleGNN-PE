"""
Fixed (frozen) GNN embedding + GP baselines on docking (Parquet-derived molecules).
...
def _emb_cache_path(args):
    """Disk path for the frozen embedding matrix Z, or None if caching is off.

    Z depends ONLY on (target, embed_dim, embed_depth, embedder_seed) — not on the bandit
    seed or the baseline — so it is identical across every seed/baseline run and can be
    computed once per target and reused. This mirrors the scan-indexer cache used by the
    GNN-PE runner, so both methods amortize their one-time precompute for a fair runtime
    comparison. Caching is byte-identical (np.save/np.load of the same array): it changes
    only runtime, never the numbers."""
    if getattr(args, "emb_cache_dir", None) and getattr(args, "target", None):
        return os.path.join(
            args.emb_cache_dir,
            f"emb_{args.target}_d{args.embed_dim}_L{args.embed_depth}_s{args.embedder_seed}.npy")
    return None


def build_fixed_embeddings(graphs, feat_dim, args):
    """Compute fixed (frozen) GNN embeddings for the whole finite action set."""
    cache = _emb_cache_path(args)
    if cache is not None:
        os.makedirs(args.emb_cache_dir, exist_ok=True)
        if os.path.exists(cache):
            Z = np.load(cache)
            if Z.shape[0] == len(graphs):
                return Z
            # Stale (domain size changed) -> fall through and recompute/overwrite.
...
class IncrementalRBF_GP:
    ...
    def __init__(self, length_scale: float, noise_var: float, output_scale: float = 1.0, jitter: float = 1e-8,
        ...
        # Optional GPU path for the O(n*N) per-round candidate scoring (heavy at N=1M).
        # The incremental Cholesky stays on CPU (tiny, n<=T); only predict() runs on GPU.
        # Same math as the NumPy path -> identical results, just faster.
        self.tdev = torch_device
        self.Zt = None    # (N,d) candidate embeddings cached on GPU (float64)
        self.Zt2 = None   # (N,) squared norms of candidates

    def set_candidates(self, Z: np.ndarray):
        """Upload the fixed candidate matrix to the GPU once (reused every round)."""
        if self.tdev is None:
            return
        self.Zt = torch.as_tensor(np.asarray(Z, dtype=np.float64), device=self.tdev)
        self.Zt2 = (self.Zt * self.Zt).sum(dim=1)  # (N,)

    def _predict_gpu(self):
        dev = self.tdev
        Zt, Zt2 = self.Zt, self.Zt2
        N = Zt.shape[0]
        n = self.X.shape[0]

        Xt = torch.as_tensor(self.X, device=dev)             # (n,d)
        Lt = torch.as_tensor(self.L, device=dev)             # (n,n) lower-tri
        yt = torch.as_tensor(self.y, device=dev)             # (n,)

        X2 = (Xt * Xt).sum(dim=1, keepdim=True)              # (n,1)
        d2 = X2 + Zt2.unsqueeze(0) - 2.0 * (Xt @ Zt.T)       # (n,N)
        d2.clamp_(min=0.0)
        K_star = self.sigma_f2 * torch.exp(-0.5 * d2 / (self.ell ** 2))  # (n,N)

        # v = L^{-1} K_star  (L lower-triangular)
        v = torch.linalg.solve_triangular(Lt, K_star, upper=False)      # (n,N)
        var = self.sigma_f2 - (v * v).sum(dim=0)
        var = torch.clamp(var, min=1e-12)
        std = torch.sqrt(var)

        y_mean = yt.mean()
        y_std = yt.std(unbiased=False) + 1e-8

        def cho_solve(b):
            u = torch.linalg.solve_triangular(Lt, b.unsqueeze(1), upper=False)
            x = torch.linalg.solve_triangular(Lt.T, u, upper=True)
            return x.squeeze(1)

        alpha_std = (cho_solve(yt) - y_mean * cho_solve(torch.ones(n, dtype=yt.dtype, device=dev))) / y_std
        mean_std = K_star.T @ alpha_std                       # (N,)
        mean_raw = mean_std * y_std + y_mean
        return mean_raw.detach().cpu().numpy(), std.detach().cpu().numpy()
    ...
    def predict(self, Z: np.ndarray):
        ...
        # GPU fast path for the heavy per-round scoring (uses cached self.Zt).
        if self.tdev is not None and self.Zt is not None:
...
def run_one(args):
    ...
    graphs, graph_rewards, feat_dim = load_docking_domain(
    ...
    gp = IncrementalRBF_GP(
        ...
        output_scale=1.0,
        jitter=1e-8,
        torch_device=(device if torch.cuda.is_available() else None),
    )
    # Cache candidate embeddings on GPU once (reused every round) for the fast predict path.
    gp.set_candidates(Z)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fixed-GNN-embedding GP baselines on docking (Parquet-derived molecules) (no sklearn)")
    ...
    parser.add_argument("--emb_cache_dir", type=str, default=None,
                        help="Directory to cache the frozen embedding matrix Z per target. Z depends "
                             "only on (target, embed params, embedder_seed), so it is reused across all "
                             "seeds/baselines instead of recomputing the ~1M-graph forward each run. "
                             "Byte-identical cache (np.save/np.load); changes runtime only, not results.")
    ...
    # output/logging
    parser.add_argument("--exp_result_folder", type=str, default="results/docking_3CLPro_dock_norm_gp_baselines_T300")
